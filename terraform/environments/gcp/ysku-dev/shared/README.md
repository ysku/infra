...
## AWS Route53 サブドメイン委譲 (手動)

AWS Route53 上の親ドメインから、GCP の Cloud DNS Managed Zone へサブドメインを委譲するための設定です。
Terraform には AWS プロバイダーが含まれていないため、手動で設定する必要があります。

1. **ネームサーバーの確認**
   `terraform apply` 実行後の出力、または以下のコマンドで GCP 側のネームサーバーを確認します。
   ```bash
   terraform output dns_name_servers
   ```

2. **Route53 レコード作成**
   AWS コンソールで親ドメインのホストゾーンを開き、以下のレコードを作成します。
   - **レコード名**: サブドメイン名（例: `dev`）
   - **レコードタイプ**: `NS`
   - **値**: 手順1で確認したネームサーバー（4つ全て）
